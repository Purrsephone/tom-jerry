...
class TicTacToe(object):
    ...
    def __init__(self):
        ...
        # ex: 000000120 means there is an O in the top right and an X in the top middle
        ...
        self.maximizer_actions = [0,1,2,3,4,5,6,7,8]
        self.minimizer_actions = [0,1,2,3,4,5,6,7,8]
        ...
        for state in self.states:
            ...
            for max_action in self.maximizer_actions:
                for min_action in self.minimizer_actions:
                    ...
                    if state_array.count(1) == state_array.count(2) and state_array[max_action] == 0 and min_action == 0: # maximizer goes
                        state_array[max_action] = 1
                        state_row.append(self.state_array_to_num(state_array))
                    elif state_array.count(1) == 1 + state_array.count(2) and state_array[min_action] == 0 and max_action == 0: # minimizer goes
    ...
    # in row-major order from left to right, that is
    # [0,0,1,0,2,0,1,0,0] means an O in the top left, bottom-middle, and an X in the center
    ...
    def state_array_to_num(self, state_array) -> int:
        new_state = 0
...
if __name__ == "__main__":
    ...
    test6 = ttt.game_over(178) #0
    test7 = ttt.game_over(421) #0
    print(test1, test2, test3, test4, test5, test6, test7)
    
    print(ttt.state_num_to_array(2187), ttt.state_array_to_num([0, 0, 0, 0, 0, 0, 0, 1, 0]))